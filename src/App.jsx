...
const MainApp = () => {
  ...
  const getBgClass = () => {
    if (isDetailViewOpen) return 'bg-[#F8F3EC]';
    switch (activeTab) {
      case 'guide': return 'bg-[#F8F3E8] bg-heritage-pattern';
      case 'community': return 'bg-terracotta-pattern';
      case 'culture': return 'bg-teak-pattern';
      case 'map': return 'bg-turmeric-pattern';
      default: return 'bg-heritage-pattern';
    }
  };

  return (
    <div className={`min-h-screen ${getBgClass()} flex flex-col text-heritage-textDark transition-colors duration-500`}>
      {/* Main Content Area */}
